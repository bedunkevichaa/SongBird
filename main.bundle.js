...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function game(birdsData, audioDescr, audio, birdsDataEn, namesArray, namesArrayEn) {
  var startBtn = document.getElementById("open-modal");
  var ul = document.getElementsByClassName("item-list list-group")[0];
  var card = document.querySelector('.card-body');
  var descr = document.querySelector('.bird-description');
  var instruction = document.querySelector('.instruction');
  var birdImageDescr = document.querySelector('.bird-image-descr');
  var nameBird = document.querySelector('.name-bird');
  var species = document.querySelector('.species');
  var nameBirdMain = document.getElementById("name-bird-main");
  var birdImage = document.querySelector('.bird-image');
  var NextLevelBtn = document.querySelector('.btn');
  var scoreSpan = document.getElementById("score");
  var pagination = document.querySelectorAll('.page-item');
  var btnGameOver = document.querySelector('.btn-game-over');
  var modalGame = document.getElementById('modal-game');
  var modalScore = document.getElementById('modal-score');
  var finishScore = document.getElementById('finish-score');
  var counter = 0;
  var counterNext = 0;
  var score = 5;
  var overalScore = 0;
  ...
  var loadListBirds = function loadListBirds() {
    var birdsNames = document.querySelectorAll(".default");
    ...
    svgPlay.classList.remove('display-none');
    svgPause.classList.add('display-none');
    scoreSpan.innerText = '0';
    nameBirdMain.innerText = '******';
    birdImage.src = "./images/bird_layout.jpg";
    NextLevelBtn.style.background = 'grey';
    NextLevelBtn.style.cursor = 'auto';
    ...
    //Set rnd audio for question
    var arrAudio = birdsData[counter].map(function (el) {
      return el.audio;
    });
    var randomAudio = arrAudio[Math.floor(Math.random() * arrAudio.length)];
    audio.src = randomAudio;

    //Load shuffled birds to DOM
    namesArray = shuffle(birdsData[counter].map(function (el) {
      return el;
    }));
    namesArrayEn = namesArray.map(function (el) {
      var temp = birdsDataEn[counter].find(function (element) {
        return element.image === el.image;
      });
      if (temp.name) {
        //el.name = temp.name;
        return temp.name;
    ...
    namesArray = namesArray.map(function (el) {
      return el.name;
    });
    birdsNames.forEach(function (name) {
      name.remove();
    });
    if (localStorage.getItem("Language") === 'english') {
      namesArrayEn.forEach(function (name) {
  ...
  startBtn.addEventListener('click', loadListBirds);
  var loadLangBirds = function loadLangBirds() {
    var birdsNames = document.querySelectorAll(".default");
    if (localStorage.getItem("Language") === 'english') {
      for (var i = 0; i < namesArrayEn.length; i++) {
        birdsNames[i].lastElementChild.innerText = '';
        birdsNames[i].lastElementChild.innerText = "".concat(namesArrayEn[i]);
      }
    } else {
      for (var _i = 0; _i < namesArray.length; _i++) {
        birdsNames[_i].lastElementChild.innerText = '';
        birdsNames[_i].lastElementChild.innerText = "".concat(namesArray[_i]);
      }
    }
  };
  var toggle = document.getElementById("toggle");
  toggle.addEventListener('click', loadLangBirds);

  //Choose Bird - load card

  var loadClickedCard = function loadClickedCard(e) {
    var svgPlayDescr = document.getElementById('svg-play-descr');
    var svgPauseDescr = document.getElementById('svg-pause-descr');
    var errorSound = document.getElementById('errorSound');
    var winSound = document.getElementById('winSound');
    var svgPlay = document.getElementById('svg-play');
    var svgPause = document.getElementById('svg-pause');
    svgPlayDescr.classList.remove('display-none');
    svgPauseDescr.classList.add('display-none');
    if (e.target && e.target.matches("li.list-group-item")) {
      if (localStorage.getItem("Language") === 'english') {
        birdsDataEn[counterNext].forEach(function (el) {
        ...
        birdsData[counterNext].forEach(function (el) {
          if (el.name === e.target.innerText) {
            //console.log(el);
            arr = el;
          }
    ...
    audioDescr.src = arr.audio;
    birdImageDescr.src = arr.image;
    nameBird.innerText = arr.name;
    species.innerText = arr.species;
    descr.innerText = arr.description;
    if (arr.audio === audio.src && boolSound == true) {
      errorSound.pause();
      ...
      svgPauseDescr.classList.add('display-none');
      audioDescr.pause();
      audioDescr.currentTime = 0;
      e.target.firstElementChild.style.background = 'green';
      NextLevelBtn.style.background = '#EE3E38';
      NextLevelBtn.style.cursor = 'pointer';
      NextLevelBtn.disabled = false;
      nameBirdMain.innerText = arr.name;
      birdImage.src = arr.image;
      scoreSpan.innerText = overalScore + score;
      bool = true;
      boolSound = false;
    } else if (bool == false) {
      winSound.pause();
      ...
      errorSound.currentTime = 0;
      errorSound.play();
      e.target.firstElementChild.style.background = 'red';
      score--;
    }
  };
  ul.addEventListener('click', loadClickedCard);
  var nextLevel = function nextLevel() {
    var birdsNames = document.querySelectorAll(".default");
    var svgPlay = document.getElementById('svg-play');
    var svgPause = document.getElementById('svg-pause');
    overalScore += score;
    if (counterNext === 5 && overalScore < 30) {
      console.log('You win!');
      modalGame.style.display = 'none';
      modalScore.style.display = 'block';
      finishScore.innerText = "".concat(overalScore);
    } else if (counterNext === 5 && overalScore === 30) {
      var winMsg = document.querySelector('.lead');
      winMsg.innerText = "Викторина пройдена";
    } else {
      bool = false;
      boolSound = true;
      score = 5;
      counterNext++;
      console.log('Overal score: ' + overalScore);
      var activeDir = document.querySelector('.active');
      activeDir.nextElementSibling.classList.add('active');
      activeDir.classList.remove('active');
      scoreSpan.innerText = overalScore;
      pagination[counterNext].classList.add('active');
      ...
      //Set rnd audio for question
      var arrAudio = birdsData[counterNext].map(function (el) {
        return el.audio;
      });
      var randomAudio = arrAudio[Math.floor(Math.random() * arrAudio.length)];
      audio.src = randomAudio;
      ...
      //Load shuffled birds to DOM
      namesArray = shuffle(birdsData[counterNext].map(function (el) {
        return el;
      }));
      namesArrayEn = namesArray.map(function (el) {
        var temp = birdsDataEn[counterNext].find(function (element) {
          return element.image === el.image;
        });
        if (temp.name) {
          //el.name = temp.name;
  ...
  btnGameOver.addEventListener('click', loadListBirds);
}
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (game);

/***/ }),

/***/ "./src/js/modal-game.js":
/*!******************************!*\
  !*** ./src/js/modal-game.js ***!
  \******************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function modalGame(audioMain) {
  // Show Modal
  var openModalButton = document.getElementById("open-modal");
  var modalWindowOverlay = document.getElementById("modal-overlay");
  var mainPageButton = document.getElementById("main-page");
  var cenaCheckbox = document.getElementById('sound');
  var modalGame = document.getElementById('modal-game');
  var modalScore = document.getElementById('modal-score');
  var showModalWindow = function showModalWindow() {
    modalWindowOverlay.style.display = 'flex';
    audioMain.pause();
    cenaCheckbox.checked = true;
    modalGame.style.display = 'block';
    modalScore.style.display = 'none';
  };
  ...
  // Hide Modal
  var closeModalButton = document.getElementById("main-page");
  var hideModalWindow = function hideModalWindow() {
    modalWindowOverlay.style.display = 'none';
    audioMain.play();
    cenaCheckbox.checked = false;
    modalGame.style.display = 'block';
    modalScore.style.display = 'none';
  };
  closeModalButton.addEventListener("click", hideModalWindow);
  mainPageButton.addEventListener("click", hideModalWindow);

  // Hide On Blur
  var hideModalWindowOnBlur = function hideModalWindowOnBlur(e) {
    if (e.target === e.currentTarget) {
      console.log(e.target === e.currentTarget);
      hideModalWindow();
    }
  };
  mainPageButton.addEventListener("click", hideModalWindowOnBlur);
}
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (modalGame);

/***/ }),

/***/ "./src/js/player-descr.js":
/*!********************************!*\
  !*** ./src/js/player-descr.js ***!
  \********************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
var playerDescr = function playerDescr(audioDescr) {
  var btnPlayDescr = document.getElementById('btn-play-descr');
  var svgPlayDescr = document.getElementById('svg-play-descr');
  var svgPauseDescr = document.getElementById('svg-pause-descr');
  var audioPlayerDescr = document.querySelector(".audio-player-descr");
  audioDescr.addEventListener("loadeddata", function () {
    audioPlayerDescr.querySelector(".timebar-time-info .length").textContent = getTimeCodeFromNumDescr(audioDescr.duration);
    audioDescr.volume = .75;
  }, false);

  //click on timeline to skip around
  var timelineDescr = audioPlayerDescr.querySelector(".timebar");
  timelineDescr.addEventListener("click", function (e) {
    var timelineWidthDescr = window.getComputedStyle(timelineDescr).width;
    var timeToSeekDescr = e.offsetX / parseInt(timelineWidthDescr) * audioDescr.duration;
    audioDescr.currentTime = timeToSeekDescr;
  }, false);

  //click volume slider to change volume
  var volumeSliderDescr = audioPlayerDescr.querySelector(".controls .volume-slider");
  volumeSliderDescr.addEventListener('click', function (e) {
    var sliderWidthDescr = window.getComputedStyle(volumeSliderDescr).width;
    var newVolumeDescr = e.offsetX / parseInt(sliderWidthDescr);
    audioDescr.volume = newVolumeDescr;
    audioPlayerDescr.querySelector(".controls .volume-percentage").style.width = newVolumeDescr * 100 + '%';
  }, false);

  //check audio percentage and update time accordingly
  setInterval(function () {
    var progressBarDescr = audioPlayerDescr.querySelector(".timebar-bar");
    var timebarCircleDescr = audioPlayerDescr.querySelector(".timebar-circle");
    progressBarDescr.style.width = audioDescr.currentTime / audioDescr.duration * 100 + "%";
    timebarCircleDescr.style.left = audioDescr.currentTime / audioDescr.duration * 100 + "%";
    audioPlayerDescr.querySelector(".timebar-time-info .current").textContent = getTimeCodeFromNumDescr(audioDescr.currentTime);
  }, 500);
  btnPlayDescr.addEventListener("click", function () {
    if (audioDescr.paused) {
      svgPlayDescr.classList.add('display-none');
      svgPauseDescr.classList.remove('display-none');
      audioDescr.play();
    } else {
      svgPlayDescr.classList.remove('display-none');
      svgPauseDescr.classList.add('display-none');
  ...
  var soundPlayerDescr = document.getElementById('sound-player-descr');
  soundPlayerDescr.addEventListener("click", function () {
    audioDescr.muted = !audioDescr.muted;
  });

  //turn 128 seconds into 2:08
  var getTimeCodeFromNumDescr = function getTimeCodeFromNumDescr(num) {
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (playerDescr);
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
var player = function player(audio) {
  var svgPlay = document.getElementById('svg-play');
  var svgPause = document.getElementById('svg-pause');
  var svgPlayDescr = document.getElementById('svg-play-descr');
  var svgPauseDescr = document.getElementById('svg-pause-descr');
  ...
  //show play/pause button svg
  btnPlay.addEventListener('click', function () {
    svgPlay.classList.toggle('display-none');
    svgPause.classList.toggle('display-none');
  });
  btnPlayDescr.addEventListener('click', function () {
    svgPlayDescr.classList.toggle('display-none');
    svgPauseDescr.classList.toggle('display-none');
  });

  //Audioplayer & controls
  var audioPlayer = document.querySelector(".audio-player");
  //let arrAudio = birdsData.map((el) => el[0].audio)

  //const randomAudio = (arrAudio) => arrAudio[Math.floor(Math.random()*arrAudio.length)];

  audio.addEventListener("loadeddata", function () {
    audioPlayer.querySelector(".timebar-time-info .length").textContent = getTimeCodeFromNum(audio.duration);
    audio.volume = .75;
  }, false);

  //click on timeline to skip around
  var timeline = audioPlayer.querySelector(".timebar");
  ...
  //check audio percentage and update time accordingly
  setInterval(function () {
    var progressBar = audioPlayer.querySelector(".timebar-bar");
    var timebarCircle = audioPlayer.querySelector(".timebar-circle");
    progressBar.style.width = audio.currentTime / audio.duration * 100 + "%";
    timebarCircle.style.left = audio.currentTime / audio.duration * 100 + "%";
    audioPlayer.querySelector(".timebar-time-info .current").textContent = getTimeCodeFromNum(audio.currentTime);
  }, 500);
  ...
  btnPlay.addEventListener("click", function () {
    if (audio.paused) {
      svgPlay.classList.add('display-none');
      svgPause.classList.remove('display-none');
      audio.play();
    } else {
      svgPlay.classList.remove('display-none');
      svgPause.classList.add('display-none');
  ...
  var thumb = timebar.querySelector(".timebar-circle");
  thumb.onmousedown = function (event) {
    ...
    function onMouseMove(event) {
      var newLeft = event.clientX - shiftX - timebar.getBoundingClientRect().left;
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function soundToggler(audioMain) {
  var cenaCheckbox = document.getElementById('sound');
  ...
  cenaCheckbox.addEventListener('click', function () {
    myfunction();
  });
}
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (soundToggler);

/***/ }),

/***/ "./src/js/swich-lang.js":
/*!******************************!*\
  !*** ./src/js/swich-lang.js ***!
  \******************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
var swichLang = function swichLang(namesArray, namesArrayEn) {
  var textsRu = document.querySelectorAll('.ru');
  var textsEn = document.querySelectorAll('.en');
  var toggle = document.getElementById("toggle");
  var flag = document.getElementById("flag");
  if (localStorage.getItem("Language") === 'english') {
    flag.classList.remove('russian');
    flag.classList.add('english');
    flag.src = "https://flagicons.lipis.dev/flags/4x3/gb.svg";
    textsEn.forEach(function (text) {
      text.style.display = 'block';
    });
    textsRu.forEach(function (text) {
      text.style.display = 'none';
    });
  } else {
    if (flag.classList.contains('english')) {
      flag.classList.remove('english');
  ...
  function switchLang() {
    ...
    if (flag.classList.contains('english')) {
      ...
      localStorage.setItem("Language", flag.classList.value);
    }
    //ru to en
    else {
      flag.classList.remove('russian');
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (swichLang);
...
/* harmony import */ var _soundToggler_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./soundToggler.js */ "./src/js/soundToggler.js");
/* harmony import */ var _modal_game_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modal-game.js */ "./src/js/modal-game.js");
/* harmony import */ var _birds_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./birds.js */ "./src/js/birds.js");
/* harmony import */ var _birdsEn_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./birdsEn.js */ "./src/js/birdsEn.js");
/* harmony import */ var _game_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./game.js */ "./src/js/game.js");
/* harmony import */ var _player_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./player.js */ "./src/js/player.js");
/* harmony import */ var _player_descr_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./player-descr.js */ "./src/js/player-descr.js");
/* harmony import */ var _swich_lang_js__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ./swich-lang.js */ "./src/js/swich-lang.js");
//import '../assets/icons/logo-no-background.png';
...
window.addEventListener('DOMContentLoaded', function () {
  var audioMain = new Audio('../src/assets/sound/main.mp3');
  audioMain.loop = true;
  var audio = document.getElementById('audio');
  var audioDescr = document.getElementById('audio-descr');
  var namesArray = [];
  var namesArrayEn = [];
  (0,_swich_lang_js__WEBPACK_IMPORTED_MODULE_9__["default"])(namesArray, namesArrayEn);
  (0,_soundToggler_js__WEBPACK_IMPORTED_MODULE_2__["default"])(audioMain);
  (0,_modal_game_js__WEBPACK_IMPORTED_MODULE_3__["default"])(audioMain);
  (0,_game_js__WEBPACK_IMPORTED_MODULE_6__["default"])(_birds_js__WEBPACK_IMPORTED_MODULE_4__["default"], audioDescr, audio, _birdsEn_js__WEBPACK_IMPORTED_MODULE_5__["default"], namesArray, namesArrayEn);
  (0,_player_js__WEBPACK_IMPORTED_MODULE_7__["default"])(audio);
  (0,_player_descr_js__WEBPACK_IMPORTED_MODULE_8__["default"])(audioDescr);
});
})();
